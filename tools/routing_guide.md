...
- **Usage**: Use for volume, flashlight, notifications, and core system settings.
...
1. **Prefer CLI over Skills**: If a CLI tool (e.g., `tizen-file-manager-cli list`) can achieve the same result as a Python skill, prefer the CLI tool for better performance and reliability.
...
1. **Direct handling**: Simple tool calls (volume, notifications)
