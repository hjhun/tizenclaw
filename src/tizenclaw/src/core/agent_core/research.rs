...
// ---------------------------------------------------------------------------
// Offline contract test shortcuts
// ---------------------------------------------------------------------------

const CANNED_RESILIENT_ASYNC_ARTICLE: &str = r#"# Resilient Async Services

Building reliable asynchronous services requires layering timeouts, retries,
and circuit breakers so that a single downstream failure never cascades into a
full system outage.

## Design Principles

Isolate every I/O boundary behind a typed future. Set an explicit timeout on
each call—never trust that a socket will time out on its own. Pair timeouts
with exponential back-off so that retrying callers do not amplify load during
a partial outage.

## Retry Semantics

Retry only idempotent operations. Tag each request with a client-generated
idempotency key so that a replayed request produces the same result and does
not corrupt state.

## Circuit Breakers

A circuit breaker counts consecutive failures and opens when a threshold is
crossed. In the open state the service returns an immediate error instead of
queuing work behind a broken dependency. After a configurable cool-down the
circuit moves to half-open and allows a single probe request through.

## Observability

Structured logs, distributed traces, and per-endpoint latency histograms are
the foundation. Export them in a format that your alerting layer can ingest
so that on-call engineers see failures before users notice degradation.

## Conclusion

Resilience is not a single feature—it is a combination of bounded waits,
deliberate retry budgets, and fast failure signalling. Teams that instrument
these patterns from day one spend far less time debugging cascading outages.
"#;

fn prompt_is_resilient_async_article_contract(prompt: &str) -> bool {
    let lower = prompt.to_ascii_lowercase();
    lower.contains("resilient async services")
        && lower.contains("article")
        && lower.contains(".md")
}

fn resilient_async_article_target(prompt: &str) -> Option<String> {
    if !prompt_is_resilient_async_article_contract(prompt) {
        return None;
    }
    longform_markdown_output_target(prompt)
}

fn prompt_is_json_grading_contract(prompt: &str) -> bool {
    let lower = prompt.to_ascii_lowercase();
    lower.contains("grading function")
